    ...
    <div>
      ...
      <div style={{
        ...
        <span style={{
          ...
          {f("topic_tables_threshold_min_comments", { min: THRESHOLDS.MIN_COMMENTS })}
          {f("topic_tables_threshold_consensus", { val: (THRESHOLDS.MIN_CONSENSUS * 100) })}
          {f("topic_tables_threshold_participation", { val: (THRESHOLDS.MIN_GROUP_PARTICIPATION * 100) })}
        </span>
      </div>
      ...
      {layerEntries
        ...
        .map(([layerId, topics]) => {
          ...
          if (layerNum === 0) {
            layerName = f("topic_tables_layer_fine");
            layerDescription = f("topic_tables_layer_fine_desc");
          } else if (layerNum === totalLayers - 1) {
            layerName = f("topic_tables_layer_coarse");
            layerDescription = f("topic_tables_layer_coarse_desc");
          } else {
            layerName = f("topic_tables_layer_medium");
            layerDescription = f("topic_tables_layer_medium_desc");
          }

          const layerLabel = `${layerName}: ${f("topic_tables_count_topics", { count: topicCount })}\n${layerDescription}`;
                ...
                <thead>
                  <tr style={{ borderBottom: "2px solid #333" }}>
                    <th style={{ padding: "10px", textAlign: "left", cursor: "pointer", userSelect: "none", width: "35%" }}
                      ...
                      {f("topic_tables_col_topic")} {sortConfig.key === 'topic_name' && (sortConfig.direction === 'desc' ? '↓' : '↑')}
                    </th>
                    <th style={{ padding: "5px", textAlign: "right", cursor: "pointer", userSelect: "none", whiteSpace: "nowrap" }}
                      onClick={() => handleSort('comment_count')}>
                      {f("topic_tables_col_comments")} {sortConfig.key === 'comment_count' && (sortConfig.direction === 'desc' ? '↓' : '↑')}
                    </th>
                    <th style={{ padding: "5px", textAlign: "right", cursor: "pointer", userSelect: "none", whiteSpace: "nowrap" }}
                      onClick={() => handleSort('total_votes')}>
                      {f("topic_tables_col_total_votes")} {sortConfig.key === 'total_votes' && (sortConfig.direction === 'desc' ? '↓' : '↑')}
                    </th>
                    <th style={{ padding: "5px", textAlign: "right", cursor: "pointer", userSelect: "none", whiteSpace: "nowrap" }}
                      onClick={() => handleSort('vote_density')}>
                      {f("topic_tables_col_avg_votes")} {sortConfig.key === 'vote_density' && (sortConfig.direction === 'desc' ? '↓' : '↑')}
                    </th>
                    <th style={{ padding: "5px", textAlign: "right", cursor: "pointer", userSelect: "none", whiteSpace: "nowrap" }}
                      onClick={() => handleSort('group_consensus')}
                      title="Group-aware consensus from PCA2">
                      {f("topic_tables_col_group_consensus")} {sortConfig.key === 'group_consensus' && (sortConfig.direction === 'desc' ? '↓' : '↑')}
                    </th>
                    <th style={{ padding: "10px", textAlign: "center", whiteSpace: "nowrap" }}>{f("topic_tables_col_action")}</th>
                  </tr>
                </thead>
                <tbody>
                      ...
                      <tr key={clusterId} style={{ borderBottom: "1px solid #ccc" }}>
                        ...
                        <td style={{ padding: "5px", textAlign: "center" }}>
                          <div style={{ display: 'flex', gap: '5px', justifyContent: 'center', alignItems: 'center' }}>
                            ...
                            >
                              {f("topic_tables_btn_collective")}{statementCheck.canGenerate && ` (${statementCheck.count})`}
                            </button>
